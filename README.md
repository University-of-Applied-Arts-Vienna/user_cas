...
1. Download the current stable release from [the github releases page](https://github.com/example/user_cas/releases) according to your platform (ownCloud or Nextcloud) or use the link provided on [apps.nextcloud.com](https://apps.nextcloud.com/apps/user_cas) for Nextcloud.
...
**LDAP Deactivation Filter**: Optional second LDAP filter used during `cas:import-users-ad --delta-update` to find users that should be disabled locally even if they no longer match the main sync filter. Default: empty
...
**Exclude Group DNs**: Optional list of DN fragments, one per line or separated by `;`. Matching raw LDAP group DNs are ignored during import before group names are resolved. Default: empty
...
* cas
    ...
    * cas:debug-user-ad (Shows the LDAP data and mapped import payload for one person.)

Example:

```bash
php occ cas:debug-user-ad 3CC0BA42561343A7907DC977F9B6162F -a employeenumber
```
...
Please contribute bug reports and feedback to [GitHub Issues](https://github.com/example/user_cas/issues).  
