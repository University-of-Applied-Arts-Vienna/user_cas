...
class UserService
{
    ...
    public function updateGroups($user, $groups, $protectedGroups = '', $justCreated = false)
    {
        ...
        $normalizedGroups = $this->normalizeGroupList($groups);
        $normalizedProtectedGroups = $this->normalizeGroupList($protectedGroups);
        ...
        if (!$justCreated) {
            ...
            foreach ($oldGroups as $group) {
                ...
                if ($group instanceof \OCP\IGroup) {
                    ...
                    if (!in_array($groupId, $normalizedProtectedGroups, true) && !in_array($groupId, $normalizedGroups, true)) {
     ...
     * @return array
     */
    private function normalizeGroupList(array $groups)
    {
        $normalizedGroups = [];
