...
<form id="user_cas" class='section' method="post">
	...
	&nbsp;<?php p(\OC::$server->getAppManager()->getAppInfo('user_cas')["version"]); ?></h2>
    <div id="casSettings" class="personalblock">
        ...
        <fieldset id="casSettings-6">
            ...
            <p><label for="cas_import_ad_deactivate_filter"><?php p($l->t('LDAP Deactivation Filter')); ?></label>
                <input
                        id="cas_import_ad_deactivate_filter"
                        name="cas_import_ad_deactivate_filter"
                        value="<?php print_unescaped($_['cas_import_ad_deactivate_filter']); ?>" placeholder="(&(objectCategory=user)(userAccountControl:1.2.840.113556.1.4.803:=2))"/>
                <span class="csh"><?php p($l->t('Optional. During delta update, users returned by this filter are disabled locally even if they no longer match the main sync filter.')); ?></span>
            </p>
            ...
            <p><label for="cas_import_map_groups_exclude_dn_filter"><?php p($l->t('Exclude Group DNs')); ?></label>
                <input
                        id="cas_import_map_groups_exclude_dn_filter"
                        name="cas_import_map_groups_exclude_dn_filter"
                        value="<?php p($_['cas_import_map_groups_exclude_dn_filter']); ?>" placeholder="OU=DoNotSync,DC=mydomain,DC=com;OU=Hidden,DC=mydomain,DC=com"/>
                <span class="csh"><?php p($l->t('Optional. One DN fragment per line or separated by ;. Matching group DNs will be ignored during import.')); ?></span>
            </p>
